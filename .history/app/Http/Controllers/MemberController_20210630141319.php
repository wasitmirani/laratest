...
class MemberController extends Controller
{
    ...
    public function index(){
        ...
        $up_commings = BookingTour::with('user')->with('location')->with('package')->where([['booking_date','<=',now()],['user_id','=',$id],['booking_status','=',3]])->get();
        ...
        $total_tours = BookingTour::with('user')->with('location')->with('package')->where([['booking_date','<=',now()],['booking_status','=',2],['user_id','=',$id]])->count();
    ...
    public function getTours(){
        ...
        $tours = BookingTour::with('user')->with('location')->with('package')->where([['booking_date','<=',now()],['booking_status','=',2],['user_id','=',$id]])->get();
        return view('member.pages.tourlist',compact('tours'));
    }
    public function getBookings(){
        $id = Auth::user()->id;

        $bookings = BookingTour::with('user')->with('location')->with('package')->where([['user_id','=',$id]])->get();
    ...
    public function upcomingTours(){
        ...
        $upcommings = BookingTour::with('user')->with('location')->with('package')->where([['booking_date','>=',now()],['user_id','=',$id],['booking_status','=',2]])->get();
